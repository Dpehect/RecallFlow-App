...
export default function VocabPage() {
  ...
  const filteredRawPacks = RECALLFLOW_ENTERPRISE_DATA.vocabPacks.filter(item => {
    ...
    const searchMatch = !searchQuery || 
      ...
      item.exampleTarget.toLowerCase().includes(searchQuery.toLowerCase());
    return langMatch && catMatch && levelMatch && searchMatch;
  });
  ...
  // Direct Toggle Mastery from Table View Mode
  const toggleMasteryDirect = async (item: SRSItem) => {
    const isCurrentlyMastered = item.state === 'mastered';
    const grade: ReviewGrade = isCurrentlyMastered ? 1 : 4;
    const updatedItem = calculateSM2(item, grade);

    const newSrsMap = { ...srsItems, [updatedItem.id]: updatedItem };
    setSrsItems(newSrsMap);
    await saveStoredItem(updatedItem);

    const updatedStats = await recordReview(!isCurrentlyMastered);
    setStats(updatedStats);
  };
  ...
  }, [isFlipped, handleGrade, viewMode]);
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between bg-[#F4F1EA] text-[#141413]">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 space-y-8 flex-1 w-full">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 bg-[#FAF8F5] p-4 border-2 border-black shadow-[4px_4px_0px_0px_#121212] font-mono">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 border-2 border-black bg-white text-xs font-bold text-black focus:outline-none focus:bg-[#FAF8F5] shadow-[2px_2px_0px_0px_#121212]"
            />
          </div>
        </div>
        ...
        <div className="space-y-3 font-mono">
          <div className="flex justify-between items-center text-xs font-bold uppercase">
            ...
            <span className="text-slate-600">Filtrelenen Toplam {totalFilteredCount} Kelime</span>
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 bg-[#FAF8F5] p-4 border-2 border-black shadow-[4px_4px_0px_0px_#121212] font-mono">
          ...
          <div className="flex items-center space-x-4">
            <div className="text-xs font-black text-white bg-[#EA580C] px-3 py-1 border border-black flex items-center gap-1.5 uppercase">
              <Sparkles className="w-3.5 h-3.5" /> MİNİMUM 600 KELİME DİL HAVUZU (TOPLAM {totalFilteredCount})
            </div>
            ...
            <div className="flex items-center space-x-1.5">
              <button
                ...
                className="p-1.5 bg-white border-2 border-black disabled:opacity-40 font-black shadow-[2px_2px_0px_0px_#121212]"
              >
        ...
        {/* MAIN DISPLAY AREA: CARD MODE VS TABLE/LIST MODE */}
        {viewMode === 'table' ? (
          /* TABLE / SÖZLÜK MODU DISPLAY */
          <div className="bg-[#FAF8F5] border-2 border-black p-6 shadow-[6px_6px_0px_0px_#121212] font-mono space-y-4">
            <div className="flex justify-between items-center border-b-2 border-black pb-3">
              <h2 className="font-editorial text-2xl font-black text-black italic">
                Sözlük / Liste Görünümü ({totalFilteredCount} Kelime)
              </h2>
              <span className="text-xs font-bold text-slate-700">Sayfa {currentPage} / {totalPages}</span>
            </div>

            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse border-2 border-black">
                <thead>
                  <tr className="bg-[#EAB308] text-black border-b-2 border-black text-xs font-black uppercase">
                    <th className="p-3 border-r-2 border-black">Seviye</th>
                    <th className="p-3 border-r-2 border-black">Kelime & Okunuş</th>
                    <th className="p-3 border-r-2 border-black">Tür</th>
                    <th className="p-3 border-r-2 border-black">Türkçe Anlamı</th>
                    <th className="p-3 border-r-2 border-black">Örnek Cümle</th>
                    <th className="p-3 border-r-2 border-black">Ses</th>
                    <th className="p-3">SM-2 Durumu</th>
                  </tr>
                </thead>
                <tbody className="divide-y-2 divide-black text-xs font-bold">
                  {currentDeck.map((item, idx) => {
                    ...
                    return (
                      <tr key={item.id || idx} className="hover:bg-white transition bg-[#FAF8F5]">
                        <td className="p-3 border-r-2 border-black whitespace-nowrap">
                          <span className="bg-[#121212] text-white px-2 py-0.5 border border-black font-black">
                            {item.level}
                          </span>
                        </td>
                        ...
                        <td className="p-3 border-r-2 border-black max-w-xs">
                          ...
                          <div className="text-[10px] text-slate-600 font-normal mt-0.5">→ {item.exampleTranslation}</div>
                        </td>
                        <td className="p-3 border-r-2 border-black whitespace-nowrap">
                          <button
                        ...
                        <td className="p-3 whitespace-nowrap">
                          <button
                            onClick={() => toggleMasteryDirect(item)}
                            className={`px-3 py-1.5 border-2 border-black font-black uppercase text-[10px] shadow-[2px_2px_0px_0px_#121212] flex items-center gap-1.5 transition ${
                              isMastered
                                ? 'bg-[#4ADE80] text-black'
                                : 'bg-white text-black hover:bg-[#F2EFE9]'
                            }`}
                          >
                            {isMastered ? <Check className="w-3.5 h-3.5 text-black" /> : <RefreshCw className="w-3.5 h-3.5 text-black" />}
                            <span>{isMastered ? 'Öğrenildi' : 'Öğrenilmedi'}</span>
                          </button>
                        </td>
                      </tr>
                    );
            ...
            <div className="flex justify-between items-center pt-2 font-mono">
              <span className="text-xs font-bold text-slate-700">Görüntülenen: {currentDeck.length} / {totalFilteredCount}</span>
              <div className="flex items-center space-x-2">
                <button
                  disabled={currentPage === 1}
                  onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))}
                  className="px-3 py-1 bg-white border-2 border-black disabled:opacity-40 font-black text-xs shadow-[2px_2px_0px_0px_#121212]"
                >
                  Önceki Sayfa
                </button>
      ...
      <footer className="bg-[#FAF8F5] border-t-2 border-black py-6 text-center text-xs font-mono font-bold text-slate-800">
        RECALLFLOW EDITORIAL SÖZLÜK & TOPLU LİSTE GÖRÜNÜMÜ SİSTEMİ
      </footer>
    </div>
  );
}
