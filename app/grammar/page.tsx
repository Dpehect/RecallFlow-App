...
export default function GrammarPage() {
  ...
  const guides: GrammarLesson[] = (RECALLFLOW_ENTERPRISE_DATA.grammarGuides || []).filter((g: GrammarLesson) => g.language === selectedLang);
    ...
    <div className="min-h-screen flex flex-col justify-between bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8 flex-1 w-full">
        <div className="border-b border-slate-200 dark:border-slate-800 pb-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div>
            <span className="text-xs font-mono text-blue-600 dark:text-blue-400 font-bold uppercase tracking-wider">BÖLÜM 2</span>
            <h1 className="text-3xl font-black text-slate-900 dark:text-white mt-1">Dil Bilgisi ve Cümle Yapısı</h1>
            <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">Gramer kurallarını karmaşık açıklamalar olmadan basit örneklerle kavrayın.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {guides.length === 0 ? (
            <div className="col-span-full py-16 text-center text-slate-400 font-mono text-sm bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800">
              Bu dil için dil bilgisi rehberi güncelleniyor.
            </div>
          ) : (
            guides.map((guide: GrammarLesson) => (
              <div key={guide.id} className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 shadow-md space-y-6">
                ...
                <div className="space-y-3">
                  <span className="text-xs font-bold font-mono text-slate-400 uppercase">Örnek Kullanımlar:</span>
